...
Supported schemes: `ssh://`, `sftp://`, `scp://` (cellpy’s `OtherPath`). cellpy
copies the file to a local temp dir, then opens it. Served instances refuse
remote URIs — use upload or a volume mount instead.

Credentials come from cellpy’s own env / `~/.env_cellpy` (the app already loads
them with cellpy’s config stack):
